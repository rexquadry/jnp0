...
async function initCortex(apiKey) {
	try {
		...
		resultCallback(
			"",
			"",
			"Problem setting up scanner, is your camera enabled?"
		);
	}
}
...
function getduplicateFilteringList() {
	...
	try {
		let duplicateFilteringList = document.getElementById(
			"duplicateFilteringList"
		);
		for (let i of duplicateDelays) {
			...
			opt.onclick = function () {
				...
				for (let i = 0; i < btns.length; i++) {
					if (btns[i].className == "dropdown-btn active")
						btns[i].className = "dropdown-btn";
				}
...
function iPhoneorMac() {
	return ["iPhone", "iPod", "Mac68K", "MacPPC", "MacIntel", "iPad"].includes(
		navigator.platform
	);
}
